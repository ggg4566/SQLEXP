...
class Oracle(Databases):
    ...
    def double_search(self, table_name, col_name, in_limit='1', index="", left_number=0, right_number=136):
        while True:
    ...
    def get_payload(self,table_name,col_name,i="1",index="1",value=""): # (index,vaule) is used blind
        ...
        for col in col_name:
            cols.append(col)
            cat_str = cols[0]
        boundary = self.boundary.replace('%value',value).replace('%index',index)
